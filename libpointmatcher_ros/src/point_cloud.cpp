...
rosMsgToPointMatcherCloud(const sensor_msgs::PointCloud2 &rosMsg,
  ...
  for (auto it(rosMsg.fields.begin()); it != rosMsg.fields.end();
    ...
    //        {
    //          case PF::INT8:
    //            timeView(dim, ptId) = boost::uint64_t(*reinterpret_cast<const
    //            int8_t*>(fPtr)); fPtr += 1; ROS_INFO("!!!!!!!!!!!! CASE INT8
    //            !!!!!!!!!!!!!!!!"); break;
    //          case PF::UINT8:
    //            timeView(dim, ptId) = boost::uint64_t(*reinterpret_cast<const
    //            uint8_t*>(fPtr)); fPtr += 1; ROS_INFO("!!!!!!!!!!!! CASE UINT8
    //            !!!!!!!!!!!!!!!!"); break;
    //          case PF::INT16:
    //            timeView(dim, ptId) = boost::uint64_t(*reinterpret_cast<const
    //            int16_t*>(fPtr)); fPtr += 2; ROS_INFO("!!!!!!!!!!!! CASE INT16
    //            !!!!!!!!!!!!!!!!"); break;
    //          case PF::UINT16:
    //            timeView(dim, ptId) = boost::uint64_t(*reinterpret_cast<const
    //            uint16_t*>(fPtr)); fPtr += 2; ROS_INFO("!!!!!!!!!!!! CASE
    //            UINT16 !!!!!!!!!!!!!!!!"); break;
    //          case PF::INT32:
    //            timeView(dim, ptId) = boost::uint64_t(*reinterpret_cast<const
    //            int32_t*>(fPtr)); fPtr += 4; ROS_INFO("!!!!!!!!!!!! CASE INT32
    //            !!!!!!!!!!!!!!!!"); break;
    //          case PF::UINT32:
    //            timeView(dim, ptId) = boost::uint64_t(*reinterpret_cast<const
    //            uint32_t*>(fPtr)); fPtr += 4; ROS_INFO("!!!!!!!!!!!! CASE
    //            UINT32 !!!!!!!!!!!!!!!!"); break;
    //          case PF::FLOAT32:
    //            timeView(dim, ptId) = boost::uint64_t(*reinterpret_cast<const
    //            float*>(fPtr)); fPtr += 4;
    ...
    //          case PF::FLOAT64:
    //            timeView(dim, ptId) = boost::uint64_t(*reinterpret_cast<const
    //            double*>(fPtr)); fPtr += 8; ROS_INFO("!!!!!!!!!!!! CASE
    //            FLOAT64 !!!!!!!!!!!!!!!!"); break;
    //          default: abort();
    //        }
    //      }
...
typename PointMatcher<T>::DataPoints rosMsgToPointMatcherCloud(
    const sensor_msgs::LaserScan &rosMsg, const tf::TransformListener *listener,
    const std::string &fixedFrame, const bool force3D, const bool addTimestamps,
    const bool addObservationDirection) {
  typedef PointMatcher<T> PM;
  typedef typename PM::DataPoints DataPoints;
  typedef typename DataPoints::Label Label;
  typedef typename DataPoints::Labels Labels;
  ...
  const ros::Time endTime(
      startTime +
      ros::Duration(rosMsg.time_increment * (rosMsg.ranges.size() - 1)));
  ...
  for (size_t i = 0; i < ranges.size(); ++i) {
    ...
    const float dt_point =
        (rosMsg.scan_time * turnRatio) / rosMsg.ranges.size();

    if (listener) {

      const ros::Time curTime(rosMsg.header.stamp +
                              ros::Duration(ids[i] * dt_point));

      // wait for transform
      listener->waitForTransform(rosMsg.header.frame_id, fixedFrame, curTime,
                                 ros::Duration(0.1));
      ...
      try {
        listener->transformPoint(fixedFrame, rosMsg.header.stamp, pin,
                                 fixedFrame, p_out);

        if (addObservationDirection) {
          listener->transformPoint(fixedFrame, curTime, s_in, fixedFrame,
                                   s_out);
        }
      } catch (const tf::ExtrapolationException &e) {
        // ROS_WARN_STREAM("libpointmatcher_ros: Couldn't transform point: " <<
        // e.what());
        return DataPoints();
      }

      // cout << "pin: " << pin.point.x << ", " << pin.point.y << ", " <<
      // pin.point.z << endl; cout << "p_out: " << p_out.point.x << ", " <<
      // p_out.point.y << ", " << p_out.point.z << endl;
  ...
  if (addTimestamps) {
    ...
    for (size_t i = 0; i < ranges.size(); ++i) {
      const ros::Time curTime(rosMsg.header.stamp +
                              ros::Duration(ids[i] * rosMsg.time_increment));
      const boost::posix_time::time_duration duration =
          boost::posix_time::seconds(curTime.sec) +
          boost::posix_time::nanoseconds(curTime.nsec);
...
template PointMatcher<float>::DataPoints rosMsgToPointMatcherCloud<float>(
    const sensor_msgs::LaserScan &rosMsg, const tf::TransformListener *listener,
    const std::string &fixedFrame, const bool force3D, const bool addTimestamps,
    const bool addObservationDirection);
template PointMatcher<double>::DataPoints rosMsgToPointMatcherCloud<double>(
    const sensor_msgs::LaserScan &rosMsg, const tf::TransformListener *listener,
    const std::string &fixedFrame, const bool force3D, const bool addTimestamps,
    const bool addObservationDirection);
...
pointMatcherCloudToRosMsg<float>(const PointMatcher<float>::DataPoints &pmCloud,
                                 const std::string &frame_id,
                                 const ros::Time &stamp);
template sensor_msgs::PointCloud2 pointMatcherCloudToRosMsg<double>(
    const PointMatcher<double>::DataPoints &pmCloud,
    const std::string &frame_id, const ros::Time &stamp);
