...
#include "landmark_finder/LandmarkArray.h"

namespace stargazer_ros_tool {

inline stargazer::Landmark convert2Landmark(const landmark_finder::Landmark& lm_in) {
...
inline stargazer::ImgLandmark convert2ImgLandmark(const landmark_finder::Landmark& lm_in) {
...
inline std::vector<stargazer::Landmark> convert2Landmarks(const landmark_finder::LandmarkArray& lms_in) {
...
inline std::vector<stargazer::ImgLandmark> convert2ImgLandmarks(const landmark_finder::LandmarkArray& lms_in) {
...
inline landmark_finder::LandmarkArray convert2LandmarkMsg(const std::vector<stargazer::ImgLandmark>& lm_in,
    ...
    for (auto& lm : lm_in) {
        landmark_finder::Landmark landmark;
        landmark.header = header;
        landmark.id = lm.nID;

        for (auto& pt : lm.voCorners) {
            landmark_finder::LandmarkPoint lmpt;
            lmpt.u = static_cast<landmark_finder::LandmarkPoint::_u_type>(pt.x);
            lmpt.v = static_cast<landmark_finder::LandmarkPoint::_v_type>(pt.y);
